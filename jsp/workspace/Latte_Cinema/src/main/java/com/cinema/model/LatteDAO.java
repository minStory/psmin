...
public class LatteDAO {
	...
	public List<TicketingHistoryDTO> getTicketingHistory() {

		List<TicketingHistoryDTO> list = new ArrayList<TicketingHistoryDTO>();

		try {
			openConn();
			sql = "select * from ticketing_history order by ticketing_history_movie_start_date asc, ticketing_history_movie_start_time asc";
	...
	public List<TicketingHistoryDTO> getTicketingHistory(String no) {
		...
		try {
			openConn();
			sql = "select * from ticketing_history where ticketing_history_member_no = ? order by ticketing_history_movie_start_date asc, ticketing_history_movie_start_time asc";
	...
	public int insertTicketingHistory(TicketingHistoryDTO tDto) {
		...
		try {
			...
			sql = "select max(ticketing_history_no) from ticketing_history";
	...
	// 공지사항 내용에서 다음글을 눌렀을 때 넘어가지 않게하기 위해
	// 글번호의 최대값을 구하는 메서드
	public int getNoticeBoardMaxNo() {
