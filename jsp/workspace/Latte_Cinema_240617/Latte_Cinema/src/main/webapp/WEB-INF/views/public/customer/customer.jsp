					...
					<div class="notice_table">
                     ...
                     ${notice.get(i).getNotice_board_updated_date() }
						...
						<div class="notice_table_btn">
							<a href="customer?page=1">first</a><a
								href="customer?page=${page - 1 }">previous</a>
							<c:forEach begin="${startBlock }" end="${endBlock }" var="i">
								<a href="customer?page=${i}">${i}</a>
							</c:forEach>
							<c:if test="${endBlock < allPage }">
								<a href="customer?page=${page + 1 }">next</a>
								<a href="customer?page=${allPage }">last</a>
							</c:if>
						</div>
					...
					<ul class="inquiry_ul_box">
						...
						<form method="post" action="customer_insert_inquiry"
							...
							<table class="inquiry_table">
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<div>
												<input type="text" class="writer_box" name="title"
													placeholder="제목을 입력해주세요" title="문의내용 제목입력"> <span
													id="titleCounter">0/50</span>
											</div>
										</td>
									</tr>
									<tr>
										...
										<td>
											<div class="textarea_box">
												<textarea cols="10" rows="10" class="text_box"
													name="content"
								...
								<tbody>
									...
									<tr>
										...
										<c:if test="${!empty dto}">
											...
											</select> <input name="phone1" value="${snum}"> <input
												name="phone2" value="${mnum}"> <input name="phone3"
												value="${fnum}"></td>
										</c:if>
										<c:if test="${empty dto}">
											<td><select name="mobile_carrier">
											...
											</select> <input name="phone1"> <input name="phone2">
												<input name="phone3"></td>
										</c:if>
									</tr>
									<tr>
										<th scope="row">이메일</th>
							...
							<div class="text_scroll">
								...
								<p>
									의사소통 경로 확보<br>② 수집 항목<br> *필수입력사항<br> -이용자 식별을
									위한
								</p>
								<p>항목 : 성명, 연락처, 이메일, 아이디(로그인 시 수집)</p>
								<br> <br> <span class="span_red">개인정보의 보유 및
									이용기간<br> 입력하신 개인정보는 소비자 보호에 관한 법률 등 관계 법률에 의해 다음과 같이
									보유합니다.<br> 보유기간 : 문의접수 후 처리 완료시점으로 부터 3년
								</span><br> <br>
								<p>※ 1:1문의 서비스 제공을 위한 최소한의 개인정보이며 거부할 수 있습니다. 다만, 수집에 동의하지
									않을 경우 서비스 이용이 제한됩니다.</p>
							</div>
							<div class="inquiry_radio_box">
								<input type="radio" name="rdo01"><label>동의 </label><input
									type="radio" name="rdo01"><label>동의하지않음</label>
							</div>
