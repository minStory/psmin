				...
				<div align="center">
					...
					<table class="car_myreview_board_content_table" border="1"
						...
						<tr>
							...
							<td colspan="3"><textarea class="content_input_cont"
									name="content" rows="50" cols="50" readonly>${cont.getSell_review_board_cont() }</textarea>
							</td>
						</tr>
						<tr>
							<th>사진첨부</th>
							<c:if test="${!empty cont.getSell_review_board_file() }">
								<td colspan="3"><img class="car_review_img"
									src="upload_images/${cont.getSell_review_board_file() }"
									width="700px">
								</td>
							</c:if>
							<c:if test="${empty cont.getSell_review_board_file() }">
					...
					<div>
						<input class="mypage_review_btn" type="button" value="글 수정하기"
							onclick="location.href='mypage_sell_review_content_modify?no=${cont.getSell_review_board_no()}'">&nbsp;&nbsp;
						<input class="mypage_review_btn" type="button" value="글 삭제하기"
							onclick="if(confirm('해당 게시글을 삭제하시겠습니까?')) {
                          location.href='mypage_sell_review_content_delete?no=${cont.getSell_review_board_no()}'}else { return; }">
					</div>
				</div>
			</div>
