...
public class MypageSellReviewContentModifyOkAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		String sell_review_writer_id = multi.getParameter("writer_id").trim();
		String content = multi.getParameter("content").trim();
		String update = multi.getParameter("modify_date").trim();

		int board_no = Integer.parseInt(multi.getParameter("board_no"));

		File sell_file = multi.getFile("file");

		SellReviewBoardDTO dto = new SellReviewBoardDTO();

		if (sell_file != null) { // 첨부파일이 존재하는 경우

			// getName(): 첨부파일의 이름을 문자열로 반환하는 메서드,
			String fileName = sell_file.getName();
			...
			String homedir = saveFolder + "/" + sell_review_writer_id;
			...
			sell_file.renameTo(new File(homedir + "/" + reFileName));

			// 실제로 DB의 file 컬럼에 들어가는 파일 이름
			// "/회원번호/차량번호_날짜_파일명" 으로 저장할 예정
			String fileDBName = sell_review_writer_id + "/" + reFileName;

			dto.setSell_review_board_file(fileDBName);
		}

		dto.setSell_review_board_no(board_no);
		dto.setSell_review_board_title(title);
		dto.setSell_review_board_cont(content);
		dto.setSell_review_board_update(update);

		TotalDAO dao = TotalDAO.getInstance();

		int check = dao.MypageSellReviewContentModifyOk(dto);

		if (check > 0) {
			ActionForward forward = new ActionForward();

			forward.setPath("/WEB-INF/views/public/mypage/mypage_sell_review_modify_ok.jsp");
