...
public class CommunityBoardModifyOkAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		int community_no = Integer.parseInt(request.getParameter("community_board_no"));
		...
		dto.setCommunity_board_no(community_no);
		...
		if (check > 0) {
			...
			forward.setPath("/WEB-INF/views/public/boards/community_board_modify_ok.jsp");
			...
			out.println("<script>");
			out.println("alert('글 수정에 실패했습니다. 입력한 정보를 다시한번 확인하세요.')");
