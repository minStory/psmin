...
document.addEventListener("DOMContentLoaded", () => {
	...
	const dto_field = document.getElementsByClassName("dto-field")[0].children[0];
	...
	const dto_name = dto_field.getAttribute("data-name");

    // 권한 필드
	const dto_role_field = document.getElementsByClassName("dto-role-field")[0];
	const dto_role = dto_role_field.getAttribute("data-role");
    ...
    }else if(dto_role === "ROLE_USER"){
        dto_field.innerHTML = "<a href='/user/logout'>로그아웃</a>";
        name_field.innerHTML = `<a href='/user/home' class='btn-signup'>${dto_name}님</a>`;
        myinfo_field.innerHTML = `<div class='content-myinfo-top'><span>grade</span>`
    ...
    }else if(dto_role === "ROLE_ADMIN"){
        dto_field.innerHTML = "<a href='/user/logout'>로그아웃</a>";
        name_field.innerHTML = `<a href='/admin/home' class='btn-signup'>관리자님</a>`;
	...
	if(!window.location.href.includes("index") || window.location.href.includes("admin_index")){
